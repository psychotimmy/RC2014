...
22 CLEAR
23 TL=62001           : REM ** Top left corner screen memory location
24 SW=40              : REM ** Playing area width (valley, swamps and woods) in columns
26 SL=12              : REM ** Lines used by playing areas
28 ST=20              : REM ** Columns in castle playing areas
30 LF$=CHR$(10)       : REM ** Line feed
32 CS$=CHR$(27)+"[2J" : REM ** Clear screen and cursor home
34 CH$=CHR$(27)+"[H"  : REM ** Cursor home (non-destructive)
36 PRINT CS$:PRINT "The Valley (c)1982 Computing Today, RC2014 CP/M version (c)2024 Tim Holyoake":PRINT:GOSUB 1600: REM ** Anykey - Randomizes the randomizer too
38 PRINT:PRINT "Initialising game, please wait .";
39 REM ** DD$ holds 1 to 25 cursor down codes. Note RIGHT$ trims leading blank on int to str conversion
40 DIM DD$(25):FOR I=1 TO 25:DD$(I)=CHR$(27)+"[":DD$(I)=DD$(I)+RIGHT$(STR$(I),LEN(STR$(I))-1):DD$(I)=DD$(I)+"B":NEXT I
41 REM ** RR$ holds 1 to 45 cursor right codes. Note RIGHT$ trims leading blank on int to str conversion
42 DIM RR$(45):FOR I=1 TO 45:RR$(I)=CHR$(27)+"[":RR$(I)=RR$(I)+RIGHT$(STR$(I),LEN(STR$(I))-1):RR$(I)=RR$(I)+"C":NEXT I
43 REM ** UU$ holds 1 cursor up code. Note RIGHT$ trims leading blank on int to str conversion
44 DIM UU$(1):UU$(1)=CHR$(27)+"[":UU$(1)=UU$(1)+RIGHT$(STR$(1),LEN(STR$(1))-1):UU$(1)=UU$(1)+"A"
45 REM ** LL$ holds 1 to 30 cursor left codes. Note RIGHT$ trims leading blank on int to str conversion
46 DIM LL$(30):FOR I=1 TO 30:LL$(I)=CHR$(27)+"[":LL$(I)=LL$(I)+RIGHT$(STR$(I),LEN(STR$(I))-1):LL$(I)=LL$(I)+"D":NEXT I
47 PRINT "."; 
49 REM ** Scenario codes (V=Valley, W=Woods & Swamps, Y=Castles)
50 VC=73 : REM ** Character's code for all scenarios
51 VB=61 : REM ** Valley border
52 VX=64 : REM ** Valley safe castle
53 VU=47 : REM ** Valley path 'up'
54 VD=92 : REM ** Valley path 'down'
55 VW=33 : REM ** Valley woods
56 VS=45 : REM ** Valley swamps
57 VV=65 : REM ** Valley black tower
58 WB=58 : REM ** Woods/Swamps border
59 WL=35 : REM ** Woods/Swamps lake
60 WC=43 : REM ** Woods/Swamps castle
61 WW=94 : REM ** Woods tree
62 WG=126: REM ** Swamps grass tufts
63 YW=88 : REM ** Castle border and walls
64 YS=62 : REM ** Castle stairs
65 YD=95 : REM ** Castle doorway
66 YF=42 : REM ** Castle treasures
67 SP=32 : REM ** Blank/space code for all scenarios
69 REM ** Default keyboard input mask for Uniget
70 VK$="0123456789EUIOJKL"
...
150 D5$=CH$+RR$(1)+DD$(15) : REM Cursor Home + 1 right + 15 down
160 D6$=CH$+RR$(1)+DD$(16) : REM Cursor Home + 1 right + 16 down
170 D9$=CH$+RR$(1)+DD$(19) : REM Cursor Home + 1 right +19 down
180 SP$="                                       ": REM 39 blank spaces
190 PRINT ".";
...
325 PRINT ".";
329 REM ** Blank virtual screen, then compute border, valley path and scenario positions
330 GOSUB 58000:GOSUB 56310
335 PRINT " done."
340 DF=100:DL$="D":GOSUB 36000:REM ** Delay
389 REM ** Opening titles
390 GOSUB 56110
999 REM ** Character choice and load
1000 PRINT:PRINT "Load your character from disk (VALCHAR.DAT) (Y/N) ?"
1010 VG$="YN":GOSUB 1500:VG$=VK$: REM ** Uniget Y/N and reset keyboard input mask
1020 IF GC$="N" GOTO 1160
1030 PRINT CS$;"Loading character from disk ...";
1040 FI$="VALCHAR.DAT"
1050 OPEN "I",1,FI$
1060 INPUT#1,J$:INPUT#1,P$
1070 INPUT#1,TS:INPUT#1,EX:INPUT#1,TN:INPUT#1,CS:INPUT#1,PS
1080 INPUT#1,T(0):INPUT#1,T(1):INPUT#1,T(2)
1090 INPUT#1,C1:INPUT#1,P1
1099 REM ** Extended data input
1100 INPUT#1,UR:INPUT#1,UB
1110 FOR I=0 TO 18
1120 INPUT#1,MA(I)
1130 NEXT I
1140 CLOSE 1
...
1170 IF J$="" GOTO 1160
1180 IF LEN(J$)>12 THEN PRINT "Name is too long (less than or equal to 12 characters)":DF=150:DL$="D":GOSUB 36000:GOTO 1160
1240 PRINT CS$;LF$;"Character types ... choose carefully"
1250 PRINT
1260 PRINT "Wizard     (1)"
...
1500 GC$=INKEY$:IF LEN(GC$) > 0 GOTO 1505
1501 GOTO 1500
1502 REM ** Convert lower to upper case
1505 IF ASC(GC$)>=97 AND ASC(GC$)<=122 THEN GC$=CHR$(ASC(GC$)-32)
...
1610 TI=INT(RND(1)*10000):RANDOMIZE(TI):GC$=INKEY$:IF LEN(GC$) > 0 THEN RETURN
1620 GOTO 1610
1699 REM ** Combat get routine
1700 TV=0:FOR I=1 TO 10:GC$=INKEY$:NEXT I:REM ** Flush kbd buffer
1710 FOR I=1 TO 72*TM
1720 GC$=INKEY$:IF LEN(GC$) > 0 GOTO 1750
1730 NEXT I
...
1780 IF ASC(GC$)>=97 AND ASC(GC$)<=122 THEN GC$=CHR$(ASC(GC$)-32)
1790 RETURN
...
2050 FOR I = 1 TO 10:GC$=INKEY$:NEXT I: REM ** Clear kbd buffer
...
2130 IF Q1=VB OR Q1=YW OR Q1=WW THEN TN=TN-1:GOTO 2030:REM ** Wall/tree
...
2210 RF=RND(1)
2220 IF RF<0.33 GOTO 3000: REM ** Monster selection
2230 IF RF>0.75 GOTO 2300: REM ** A find!
...
2840 IF S=4 AND RN>0.7 AND T(0)=1 AND T(1)<6 AND FL>T(1) GOTO 2890
...
3020 RF=INT(RND(1)*17):IF RF>9 AND RND(1)>0.85 GOTO 3020
3030 IF Q1=WL OR PK=WL THEN RF=INT(RND(1)*2+17)
3040 IF RF=16 AND RND(1)<0.7 GOTO 3020
...
3500 IF RND(1)<0.6 GOTO 4000:REM ** Monster's combat
...
4030 IF MS<N AND N>6 AND RND(1)<0.5 GOTO 4300
...
5280 PRINT D$;"The psi lance saps ";D*3;" psi points"
...
9190 IF Q1=VW OR Q1=VS THEN D2=I:R2=P(2)
...
10029 REM ** Draw the valley frame
...
12050 FOR I=1 TO 100
12060 UY=INT(RND(1)*SL)+1:UX=INT(RND(1)*(SW-2))+1:POKE(TL+(UY*SW)+UX),PC
12070 NEXT I
12079 REM ** Poke in the lake to the virtual screen
12080 POKE TL+(D2*SW)+R2+2,WL:POKE TL+(D2*SW)+R2+3,WL
12090 FOR I=1 TO 5:POKE TL+((D2+1)*SW)+R2+I,WL:NEXT I
...
12140 FOR I=1 TO 4:POKE TL+((D2+4)*SW)+R2+I,WL:NEXT I
12150 FOR I=6 TO 7:POKE TL+((D2+4)*SW)+R2+I,WL:NEXT I
12160 FOR I=3 TO 4:POKE TL+((D2+5)*SW)+R2+I,WL:NEXT I 
12170 POKE TL+((D2+6)*SW)+R2+4,WL
12189 REM ** Draw in the frame
...
14420 PRINT CH$;RR$(24);DD$(4);"The Black Tower"
14430 PRINT RR$(27);"of Zaexon"
14440 PRINT RR$(27);DD$(3);"Floor ";FL-1:GOTO 14490
14450 PRINT CH$;RR$(27);DD$(2);"Vounim's"
14460 PRINT RR$(29);"Lair":GOTO 14500
14470 PRINT CH$;RR$(25);DD$(2);"The Temple Of"
14480 PRINT RR$(27);"Y'Nagioth"
14490 P(FL+1)=P(FL)+P
...
36150 PRINT D6$;RR$(20);"  Combat str =";CS
36160 PRINT RR$(21);"  Psi power  =";PS
36170 PRINT RR$(21);"  Stamina    =";C
...
36200 RETURN
36210 PRINT D9$;">> ";M$;" <<";
36220 PRINT D9$;RR$(20);"  M Str =";MS;N;" "
36230 RETURN
39999 REM ** Extended data display in columns 41+ for RC2014/VT100
40000 IF UE=1 THEN UE=0
40001 REM ** UE=0 so not called until next monster OR if scene
40002 REM ** changes and at least one monster defeated (UB>0)
40010 IF UB=0 THEN RETURN
40015 PRINT CH$;DD$(1);RR$(45);" Monster type & number slain":PRINT RR$(45);"                              ";
40020 FOR I=0 TO 18
40030 IF MA(I)=0 GOTO 40050 
40040 PRINT DD$(1):PRINT UU$(1);RR$(45);"                              ";LL$(30);" ";RIGHT$(M$(I),LEN(M$(I))-1);RR$(28-LEN(M$(I))-LEN(STR$(MA(I))));MA(I);
40050 NEXT I
40060 PRINT DD$(1):PRINT UU$(1);RR$(45);"                              ";
40070 RETURN 
...
48020 PRINT D$;DD$(1);"Wilt thou leave the valley (Y/N) ?"
...
50040 FI$="VALCHAR.DAT":PRINT: PRINT "Saving ";J$;" as ";FI$;"...";
50070 OPEN "O",1,FI$
50080 PRINT#1,J$: PRINT#1,P$
50090 PRINT#1,TS: PRINT#1,EX: PRINT#1,TN:PRINT#1,CS:PRINT#1,PS
50100 PRINT#1,T(0):PRINT#1,T(1):PRINT#1,T(2)
50110 PRINT#1,C1: PRINT#1,P1
50119 REM ** Extended data output
50120 PRINT#1,UR:PRINT#1,UB
50130 FOR I=0 TO 18
50140 PRINT#1,MA(I)
50150 NEXT I
50160 CLOSE 1
50200 PRINT "save complete."
...
55030 PRINT D$;"Oh what a frail shell"
...
55090 PRINT:PRINT "        ++ Live again ++"
55100 DF=150:GOSUB 36000
55110 L=G(0):MP=L:M=W:S=1:GOTO 9220
56109 REM ** Opening titles subroutine
56110 PRINT CS$;"Welcome to The Valley, brave adventurer."
56120 PRINT LF$;"Your quest is to find the Helm of Evanna"
56130 PRINT "and leave The Valley as a hero.";LF$ 
56140 PRINT "First find the Amulet of Alarian in one of the"
56150 PRINT "Temples of Y'Naigoth in the swamplands.";LF$
56170 PRINT "Once you have successfully found the amulet,"
56180 PRINT "find the six stones to complete it in"
56190 PRINT "The Black Tower of Zaexon.";LF$ 
56200 PRINT "Finally, amulet safely retrieved, travel to the woods"
56210 PRINT "and raid Vounim's Lair to find The Helm of Evanna.";LF$
56230 PRINT "Your quest is complete when you return to either castle"
56240 PRINT "at the end of the path with The Helm.";LF$
56250 PRINT "Good luck ... and watch out for the monsters!";LF$
...
56309 REM ** Draw the valley frame
...
56369 REM ** Compute the valley path and scenario positions. 
56370 M=TL+INT(RND(1)*SL+1)*SW+1
56380 L=M:MP=M:W=M:G(0)=M:G(1)=VX:POKE G(0),G(1)
56390 FOR I=2 TO 74 STEP 2
56400 IF RND(1)>0.5 GOTO 56420
...
56510 S(I)=TL+(SW*N1)+N2
56520 IF PEEK(S(I))<>SP OR PEEK(S(I)+1)<>SP GOTO 56500
56530 NEXT I
56540 RETURN
56994 REM **
56995 REM *******************************
56996 REM ** VT100 drawing subroutines **
56997 REM *******************************
56998 REM **
56999 REM ** Print virtual screen to VT100 - for valley, swamps and woods
57000 PRINT CH$:FOR I=0 TO SL+1:FOR J=0 TO SW-1:PRINT CHR$(PEEK(TL+(SW*I)+J));:NEXT J:PRINT:NEXT I
57010 RETURN
57019 REM ** Print virtual screen to VT100 - for castles
57020 PRINT CH$:FOR I=0 TO SL+1:FOR J=0 TO ST+1:PRINT CHR$(PEEK(TL+(SW*I)+J));:NEXT J:PRINT:NEXT I
57030 RETURN
...
58000 FOR I=TL TO TL+((SL+2)*SW):POKE I,SP:NEXT I
58010 RETURN
